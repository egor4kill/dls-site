...
- Панель форматирования поддерживает заголовки, жирный и курсивный текст,
  зачёркивание, ссылки, цитаты, списки и код.
- Кнопка `+` в поле текста добавляет фотографию с подписью и размером,
  галерею, видео, выделенный блок, кнопку-ссылку или таблицу.
- Видео вставляется ссылкой с YouTube, Rutube, VK Video либо прямой ссылкой на
  файл MP4/WebM. Большие видео не следует загружать в GitHub.
- Кнопка `Подготовить фото` в правом нижнем углу открывает локальный редактор:
  обрезка, пропорции, поворот, отражение, масштабирование, изменение размера и
  сжатие в WebP/JPEG/PNG. Готовый файл скачивается на компьютер, после чего его
  нужно выбрать в медиатеке CMS.
- Decap CMS и Cropper.js собираются в `admin/cms.js` и работают с сайта без
  внешнего CDN. Команда `npm run build` обновляет этот файл автоматически.
